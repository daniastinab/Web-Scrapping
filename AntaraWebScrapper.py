...
class DETIKScraper:
    ...
    def get_articles(self, response):
        ...
        for page in range(1, int(self.pages)+1):
            ...
            articles = soup.find_all('div', {"class":"wrapper__list__article"})
            for div in articles:
                d = div.find("div", {"class":"card__post__title"})
                b = div.find("div", {"class":"card__post__author-info mb-2"})
                title_element = d.find("a")
                category_element = div.find("span", {"class": "text-dark text-capitalize"})
                date_element = b.find("span", {"class": "text-dark text-capitalize"})
                href_element = div.find("a")
...
if __name__ == '__main__':
    ...
    base_url = f"https://www.antaranews.com/search"
